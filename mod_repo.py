...
class DependencyAnalyzer:
    ...
    def calculate_coupling(self, feature_files: List[str]) -> float:
        ...
        internal, external = self.get_internal_vs_external_deps(feature_files)
        total_deps = len(internal) + len(external)
        
        if total_deps == 0:
            return 0.0
        
        # Coupling = ratio of external dependencies
        return len(external) / total_deps
    ...
    def calculate_cohesion(self, feature_files: List[str]) -> float:
        ...
        # Count internal connections between feature files
        internal_connections = 0
        possible_connections = len(feature_files) * (len(feature_files) - 1)
        
        for file in feature_files:
            rel_file = os.path.relpath(file, self.repo_path)
            if rel_file in self.imports_map:
                for other_file in feature_files:
                    rel_other = os.path.relpath(other_file, self.repo_path)
                    if rel_file != rel_other:
                        # Check if file imports/uses other_file
                        for imp in self.imports_map[rel_file]:
                            if rel_other.replace('/', '.').replace('.py', '') in imp:
...
class GitHubRepoBatchAnalyzer:
    ...
    def analyze_repo(self, repo_url: str) -> ExtractabilityMetrics:
        ...
        try:
            ...
            internal, external = analyzer.get_internal_vs_external_deps(python_files)
            ...
            complexity_score = min(avg_complexity / 10, 1.0)
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = "extractability_analysis_results.csv"
