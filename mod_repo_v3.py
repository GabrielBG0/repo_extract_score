...
class DependencyAnalyzer:
    ...
    def calculate_coupling(self, feature_files: List[str]) -> float:
        """Calculate coupling score (lower is better) - excludes fundamental ML libraries"""
        if not feature_files:
            return 0.0
        
        internal, external, fundamental = self.get_internal_vs_external_deps(feature_files)
        # Don't count fundamental ML libraries in coupling calculation
        total_deps = len(internal) + len(external)
        
        if total_deps == 0:
            return 0.0
        
        # Coupling = ratio of external dependencies (excluding fundamental libs)
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = "extractability_analysis_results_v3.csv"
