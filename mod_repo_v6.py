...
class GitHubRepoBatchAnalyzer:
    ...
    def analyze_repo(self, repo_url: str) -> ExtractabilityMetrics:
        ...
        try:
            ...
            registry_score = min(
                registry_usage / 80.0, 1.0
            )  # Changed from 100 to 80 for steeper curve
            config_penalty = 0.20 if has_config_system else 0.0  # Increased from 0.15

            # Bonus for high internal dependency ratio (sign of good modularity)
            internal_ratio = len(internal) / (len(internal) + len(external) + 1)
            internal_bonus = internal_ratio * 0.10  # Up to 10 point bonus

            # Calculate overall extractability score (0-100)
            extractability = 100 * (
                (1 - coupling) * 0.20  # Reduced further
                + modularity * 0.40  # Increased - most important!
                + cohesion * 0.05  # Reduced - less discriminative
                + (1 - complexity_score) * 0.10  # Reduced
                + (1 - inheritance_score) * 0.05  # Keep same
                + (1 - registry_score) * 0.20  # Doubled! Registries kill extractability
                - config_penalty  # Bigger penalty
                + internal_bonus  # New bonus!
            )
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = "extractability_analysis_results_v6.csv"
