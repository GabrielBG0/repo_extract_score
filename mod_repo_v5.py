...
class GitHubRepoBatchAnalyzer:
    ...
    def analyze_repo(self, repo_url: str) -> ExtractabilityMetrics:
        ...
        try:
            ...
            registry_score = min(
                registry_usage / 100.0, 1.0
            )  # Changed from 50 to 100 for better scaling
            config_penalty = 0.15 if has_config_system else 0.0  # Increased from 0.1

            # Calculate overall extractability score (0-100)
            extractability = 100 * (
                (1 - coupling) * 0.25  # Reduced weight
                + modularity * 0.35  # Increased - this is key!
                + cohesion * 0.10  # Reduced weight
                + (1 - complexity_score) * 0.15  # Keep same
                + (1 - inheritance_score) * 0.05  # Reduced
                + (1 - registry_score) * 0.10  # Increased - registries are bad!
                - config_penalty  # Increased penalty
            )
...
if __name__ == "__main__":
    ...
    OUTPUT_CSV = "extractability_analysis_results_v5.csv"
